...
class Player {
    constructor(name, health, damage, healthPotions, healthPotionHealAmount, healthPotionDropChance) {
        this.name = name;
        this.health = health;
        this.damage = damage;
...
function heal(player) {
    if (player.healthPotions > 0) {
        ...
        console.log(`You healed yourself for ${player.healthPotionHealAmount} health!`);
        console.log(`You have ${player.health} health left.`);
    }
    else {
...
function game() {
    ...
    const enemies = [new Enemy('Goblin', 50, 20), new Enemy('Orc', 100, 30), new Enemy('Troll', 150, 40)];
